...
| reward_points | int | 当次签到获得的积分 |
| bonus_points | int | 额外奖励积分（连续7天+5，连续30天+15，连续365天+50） |
| max_continuous_days | int | 历史最长连续签到天数 |
| total_points | int | 用户累计总积分 |
...
| config.daily_points | int | 每日基础积分 |
...
| id | int | 记录ID |
| user_id | int | 用户ID |
| sign_in_at | string | 签到时间（ISO格式） |
| sign_in_date | string | 签到日期（YYYY-MM-DD格式） |
| continuous_day | int | 当次签到后的连续天数 |
| reward_points | int | 当次签到获得的积分 |
| ip_address | string | 签到IP |
...
| points_rankings | 按累计积分降序，相同则按累计签到次数降序 |
...
| 参数 | 类型 | 描述 |
|------|------|------|
| daily_points | int | 每日签到基础积分（选填，大于0） |
...
| sign_in_at | datetime | 签到时间（索引） |
| sign_in_date | string | 签到日期 YYYY-MM-DD（索引） |
| continuous_day | int | 当次签到后的连续天数 |
| reward_points | int | 当次签到获得的积分 |
| ip_address | string | 签到IP地址 |
...
|------|------|------|
| id | uint | 主键 |
| daily_points | int | 每日基础积分 |
| weekly_bonus | int | 连续签到触发周奖励的天数 |
| monthly_bonus | int | 连续签到触发月奖励的天数 |
| yearly_bonus | int | 连续签到触发年奖励的天数 |
| enabled | bool | 是否启用 |
